...
3. 不可变值 immutable.js
4. react-router配置懒加载 我们是Suspense和lazy
...
7. 渲染时加key
   比如 相同类型的子元素 我们需要使用唯一的key去标识他，如果少了key，其实在内部工作的时候，可能就不会去做dom
   节点交换这样的动作，会做一些性能开销更大的，直接把一个节点属性发生变化，使得跟另外一个位置的节点完全一样，另一个节点
   做同样的事情，不仅用来消除warning 更是用来提升性能
8. 自定义事件 dom事件及时销毁
9. 合理使用异步组件
10. 减少bind this次数，尽量使用类属性的方式 
11. 合理使用scu和memo
...
2. 高阶组件给表单组件 传递一个 input 组件包装函数 接管其输入事件 并同意管理表单数据
