...
- 父子组件
  props 和$emit，父组件向子组件传递数据是通过props来传递的，子组件传递数据给父组件是通过$emit 触发事件来做到的。

  补充 - 绑定的事件是绑给了儿子自己，只是绑定的事件是父组件的事件，还是相当于触发儿子自己绑定的事件，调用父类的方法

- 使用 $parent、$children 获取当前组件的父组件、当前组件的子组件
- attrs 和 listeners -》A->B->C
- 跨组件通信：父组件中通过 provider 来提供变量，然后再子组件中通过 inject 来注入变量
- 通过 $refs 获取实例
- 自定义事件 event.$on event.$off event.$emit
- vuex 状态管理

## $attrs是为了解决什么问题？应用场景有哪些？provide/inject不能解决它能解决的问题嘛？
* $attrs 主要目的就是实现批量传递数据
* provide/inject更适用于插件中，主要是实现跨层级数据传递
...
- vuex 就是创建了一个全局的 vue 实例，我们可以更改这个 vue 实例上的数据 ，达到共享的目的，所有组件都去获取这一个 vuex 中的实例数据
- action 中处理异步，mutation 不可以，去更新状态
- mutation 做原子操作
- action 可以整个多个 mutation

## vue 中的插槽是如何实现的？什么时候用它？
...
主要是缓存，采用的是 LRU 算法，最近最久未使用法。

1. 最近的就是最后一项，最久的就是第一个，每次切换都会将当前项，放入最后，缓存的是虚拟 dom

## $ref 是如何实现的？

将真实 dom 或者组件实例挂载到当前十里的$ref 属性上

## vue 中用到了哪些设计模式

- 工厂模式 - 传入参数即可创建实例(createElement),根据传入的参数不同，返回不同的实例
  比如 createElement ,tag 是 string，返回一个 vnode，tag 是一个组件，创建一个组件虚拟节点，不是，返回一个虚拟节点
- 单例模式 - 就是整个程序有且只有一个实例，比如我们的 vuex 当前已经被 install 了，在次 install 会返回以前的
- 发布订阅 - $on,$emit -> 订阅者把自己想订阅的事件订阅到调度中心，当该事件触发的时候，发布者发布该事件到调度中心，由调度中心统一调度订阅者注册到订阅中心的处理代码。
- 观察者模式 - watcher & dep 的关系
- 代理模式 - 给某一个对象提供一个代理对象，并由代理对象控制对原对象的引用 (比如 vue3 中的 proxy，就是一个代理模式)
- 装饰器模式 - @装饰器用法，对原方法进行包装
- 中介者模式 - (通过提供一个统一的接口让系统的不同部分进行通信) vuex，整个系统或者多个系统，都可以通过一个接口来通信
- 策略模式 - (根据不同的属性，去采用不同的策略去加载)指对象有某个行为，但是在不同的场景中，该行为有不同的实现方案
- 外观模式、迭代器模式、适配器模式、模板方法模式

## vue3 和 vue2 的区别

1. 对 ts 的支持，因为 vue2 使用 flow 写的，对 ts 支持不友好，比如所有属性都在 this 上，难以推导组件的数据类型
2. 大量的 API 挂载到 vue 对象的原型上，难以实现 treeShaking
3. 对跨平台支持比较友好，vue2 中有 web，weex，vue3 中专门做了一层，可以让不同的人去改写底层的渲染原理
4. Composition Api
5. 对虚拟 dom 进行了重写，对一些模板的编译也做了优化
...
1. nextTick 中的回调是在下次 dom 更新循环结束之后执行的延迟回调。在修改数据之后，立即使用这个方法，
...
用来使用插件的，我们可以在插件中拓展全局组件，指令，原型方法

## vue-router 有几种钩子函数，具体是什么以及执行流程
...
补充 - 简单来说就是弄成一个对象，当组件或者元素创建的时候，回去调用这些钩子方法，比如 bind,update 方法

## 组件中写name选项有哪些好处以及作用？
* 可以通过名字找到对应的组价 - 递归
* 可以通过name实现缓存功能 - keep-alive
* 可以通过 name 来识别组件 - 跨级组件通信时

```js
// 把自己记录到选项中，渲染的时候可以通过名字去找到当前组件
...
- 合理使用 v-show,v-if
- 合理使用 computed
- v-for 时加 key，以及避免和 v-if 同时使用，因为 v-for 优先级更高，每次循环都要 v-if，是对性能的一种浪费。因为解析的时候要先解析v-for,再解析v-if
- 自定义事件，dom 事件即时销毁
- 合理使用异步组件，keep-alive
- data 层级不要太深
- webpack 层面优化
