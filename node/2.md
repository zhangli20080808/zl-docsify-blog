...
2. 加载模块时，为什么每个模块都有__dirname,__filename属性呢，new Module的时候我们看到1.1部分没有这两个属性的，那么这两个属性是从哪里来的?

也就是说，每个module里面都会传入__filename, __dirname参数，这两个参数并不是module本身就有的，是外界传入的-原因，是内容包装成了函数

3. exports.xxx=xxx和Module.exports={}有什么区别吗
4. Node事件循环的流程
5. V8的垃圾回收机制
6. Buffer模块
7. Buffer.alloc和Buffer.allocUnsafe的区别