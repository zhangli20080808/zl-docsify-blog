...
- 宏任务：宿主提供的异步方法 ,包括 setTimeout 、setInterval 、DOM 事件click、 AJAX 、UI rendering、I/O、script
- 微任务：语言标准提供的 ,Promise（对于前端来说）Async Await 包括 promise.then 回调，MutationObserver(可以监控dom的变化，等待dom内容变化之后才回去执行)，process.nextTick，Object.observe
